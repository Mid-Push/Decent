...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = GlobalImageDis(input_dim=3)
...
class PatchDensityEstimator(nn.Module):
    def __init__(self, opt, gpu_ids=[]):
        super(PatchDensityEstimator, self).__init__()
        self.register_buffer("initialized", torch.tensor(0, dtype=torch.uint8))
        self.gpu_ids = gpu_ids
        self.opt = opt

    def create_flow(self, feats):
        opt = self.opt
        for mlp_id, feat in enumerate(feats):
            input_nc = feat.shape[1]
            if opt.flow_type == 'maf':
                print('[*] using MAF')
                flow = MAF(input_nc, n_blocks=opt.flow_blocks, hidden_size=opt.maf_dim, n_hidden=opt.maf_layers)
            elif opt.flow_type == 'bnaf':
                print('[*] using BNAF')
                flow_blocks, bnaf_layers, bnaf_dim = opt.flow_blocks, opt.bnaf_layers, opt.bnaf_dim
                flow = BNAF(input_nc, n_flows=flow_blocks, n_layers=bnaf_layers, hidden_dim=bnaf_dim)
            elif opt.flow_type == 'nsf':
                print('[*] using NSF')
                flow = NSF(input_nc, opt.flow_blocks)
            else:
                print('[*] using MAFMOG')
                flow = MAFMOG(opt.flow_blocks, opt.maf_comps, input_nc, opt.maf_dim, opt.maf_layers, input_order='random')
